...
NuvioTV is a Kotlin Multiplatform / Compose Multiplatform mobile media hub app for Android and iOS. It uses the Stremio addon ecosystem for content. The Gradle project has a shared Kotlin Multiplatform module (`:composeApp`) and an Android application module (`:androidApp`).

### Prerequisites

- **JDK 21** (pre-installed on Cloud VMs)
- **Android SDK** with platform 37.0 and build-tools. `ANDROID_HOME` must be set to the SDK location (e.g. `/opt/android-sdk`).
...
| Task | Command |
|---|---|
| Build playstore debug APK | `./gradlew :androidApp:assemblePlaystoreDebug` |
| Build full debug APK | `./gradlew :androidApp:assembleFullDebug` |
| Run unit tests | `./gradlew -Pnuvio.android.distribution=playstore :composeApp:testAndroidHostTest` |
| Kotlin compile check | `./gradlew -Pnuvio.android.distribution=playstore :composeApp:compileAndroidMain` |
| Android lint | `./gradlew :androidApp:lintPlaystoreDebug` |
...
- **Android lint has pre-existing failures**: The codebase has ~41 lint errors and ~139 warnings with no baseline configured. Lint will exit non-zero. Use Kotlin compilation (`compileAndroidMain`) as the primary code-correctness check.

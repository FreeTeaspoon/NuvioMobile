...
class PluginSyncTest {
    ...
    @Test
    fun backupImportReloadsPluginsAndRetainsRemoteDeletionBehavior(): Unit = runBlocking {
        seedPlugins()
        val backup = PluginRepository.exportPayload(1)
        val otherUrl = server.url("/other/manifest.json").toString()
        seedPlugins(listOf(otherUrl))
        PluginRepository.initialize()
        assertEquals(listOf(otherUrl), PluginRepository.uiState.value.repositories.map { it.manifestUrl })

        PluginRepository.importPayload(1, backup)
        PluginRepository.initialize()

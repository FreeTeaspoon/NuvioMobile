...
actual class InteractiveHighlight actual constructor(
    ...
    actual val modifier: Modifier =
        Modifier.drawWithContent {
            ...
            if (progress > 0f) {
                drawRect(
                    highlightColor.copy(alpha = highlightColor.alpha * 0.34f * progress),
                    blendMode = BlendMode.Plus
                )
                shader.apply {
                    val position = position(size, positionAnimation.value)
                    setFloatUniform("size", size.width, size.height)
                    setColorUniform("color", highlightColor.copy(alpha = highlightColor.alpha * progress).toArgb())
...
actual fun isInteractiveHighlightSupported(): Boolean =

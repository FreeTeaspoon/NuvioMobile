...
internal class AndroidDownloadScheduler(val context: Context) {
        ...
        try {
            ...
            val partial = if (destination.isFile) destination else transferAndroidDownload(
                ...
                onProgress = { bytes, total ->
                    ...
                    val update = progressTracker.update(bytes, total, now)
                    if (update.publishUi) {
                        val current = updateActive(transfer, persist = update.persist) {
                            it.copy(item = it.item.copy(downloadedBytes = bytes, totalBytes = total))
                        }
                        if (update.notify && current != null) onProgress(current)
                    }
                },
            )
    ...
    ): AndroidDownloadTransfer? = store.update(transfer.item.fileName, transfer.generation, persist) {

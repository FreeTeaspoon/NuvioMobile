...
check 'system media controls runtime binding' 'controller.updateNowPlayingMetadata\(buildNowPlayingInfo\(\)\)' "$common/features/player/PlayerScreenRuntimeEffects.kt"
check 'system media controls Android session' 'MediaSession\(appContext, NOW_PLAYING_TAG\)' "$android/features/player/PlayerNowPlayingController.android.kt"
